...
The public store refuses the catalog while any visibility block exists. Listing and route suppressions remove matching organic/sponsored documents and facet membership. Media suppression omits only the exact media child. Route/detail reads use the Catalog-selected public response mode: not found, gone, or temporarily unavailable. Temporary suppressions use the owner interval `[starts_at_utc, expires_at_utc)`.

Stable media identity is present in the current publication contract. Contact and external-reference suppression remain fail-closed at the Catalog command boundary because those stable public identities are not yet present end-to-end; Query also rejects such an event while retaining its durable block.
...
- Domain/application tests cover immutable components, exact suppression mapping, deterministic safety digests, replay/conflict, validation, and component preservation.
...
- Public reads are safety-filtered for organic, sponsored, facets, routes, and media and fail with typed unavailable state while blocked.
