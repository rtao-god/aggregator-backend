internal static class MigrationTemplateWriter
{
    public static void Write(CatalogMediaGenerationContext context)
    {
        ArgumentNullException.ThrowIfNull(context);
        Directory.CreateDirectory(context.MigrationDirectory);
        var catalogMigrationDirectory = Path.Combine(
            context.Root.FullName,
            "src",
            "Catalog",
            "Catalog.Migrations");
        var projectTemplate = Path.Combine(catalogMigrationDirectory, "Catalog.Migrations.csproj");
        var programTemplate = Path.Combine(catalogMigrationDirectory, "Program.cs");
        if (!File.Exists(projectTemplate) || !File.Exists(programTemplate))
        {
            throw new InvalidOperationException("Catalog migration template is unavailable.");
        }
        var catalogSql = string.Join(
            Environment.NewLine,
            Directory.GetFiles(Path.Combine(catalogMigrationDirectory, "Migrations"), "*.sql")
        ...
        foreach (var required in new[]
        {
            ...
            "media_id",
            "listing_revision_id",
            "catalog.publication_entry",
        })
        {
            if (!catalogSql.Contains(required, StringComparison.OrdinalIgnoreCase))
            {
                throw new InvalidOperationException(
                    $"Catalog migration contract '{required}' is unavailable for media publication integration.");
            }
        }

        var project = File.ReadAllText(projectTemplate)
            .Replace("Catalog.Migrations", "Catalog.Media.Migrations", StringComparison.Ordinal);
        File.WriteAllText(
            Path.Combine(context.MigrationDirectory, "Catalog.Media.Migrations.csproj"),
            project);
        var program = File.ReadAllText(programTemplate)
            .Replace("Catalog", "CatalogMedia", StringComparison.Ordinal)
            .Replace("CATALOG", "CATALOG_MEDIA", StringComparison.Ordinal);
        File.WriteAllText(Path.Combine(context.MigrationDirectory, "Program.cs"), program);
        var migrations = Path.Combine(context.MigrationDirectory, "Migrations");
        Directory.CreateDirectory(migrations);
        File.WriteAllText(
            Path.Combine(migrations, "V001__catalog_media_owner_schema.sql"),
            SchemaSql().Trim() + Environment.NewLine);
        File.WriteAllText(
            Path.Combine(migrations, "V002__catalog_media_publication_gate.sql"),
            PublicationGateSql().Trim() + Environment.NewLine);
    }
    ...
    private static string PublicationGateSql() =>
        """
        DO $$
        BEGIN
            IF EXISTS
