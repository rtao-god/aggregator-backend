...
public sealed class SourceVerificationProofWindowsEntrypointTests
{
    [Fact]
    public void WindowsEntrypointDelegatesToCanonicalPythonOrchestrator()
    {
        var source = Read("tools/run-source-verification-proof.ps1");

        Assert.Contains(
            "run-source-verification-proof.py",
            source,
    ...
    public void WindowsEntrypointRequiresPythonThreeAndPropagatesFailure()
    {
        ...
        Assert.Contains(
            "Python 3 is required to execute the source verification proof.",
            source,
            StringComparison.Ordinal);
        Assert.Contains(
            ...
            StringComparison.Ordinal);
        Assert.Contains(
            "Source verification proof failed with exit code $LASTEXITCODE.",
            source,
            StringComparison.Ordinal);
    }
