...
public sealed class RuntimeSmokeProofWindowsEntrypointTests
{
    [Fact]
    public void WindowsEntrypointDelegatesToCanonicalRuntimeSmokeOwner()
    {
        var source = Read("tools/run-runtime-smoke-proof.ps1");

        Assert.Contains(
            "run-runtime-smoke-proof.py",
            source,
    ...
    public void WindowsEntrypointRequiresPythonThreeAndPropagatesFailure()
    {
        ...
        Assert.Contains(
            "Python 3 is required to execute the runtime smoke proof.",
            source,
            StringComparison.Ordinal);
        Assert.Contains(
            ...
            StringComparison.Ordinal);
        Assert.Contains(
            "Runtime smoke proof failed with exit code $LASTEXITCODE.",
            source,
            StringComparison.Ordinal);
    }
