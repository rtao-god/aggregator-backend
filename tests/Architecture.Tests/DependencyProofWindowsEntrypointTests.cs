...
public sealed class DependencyProofWindowsEntrypointTests
{
    [Fact]
    public void WindowsEntrypointDelegatesToCanonicalPythonOrchestrator()
    {
        var source = Read("tools/run-dependency-proof.ps1");

        Assert.Contains(
            "run-dependency-proof.py",
            source,
    ...
    public void WindowsEntrypointRequiresPythonThreeAndPropagatesFailure()
    {
        ...
        Assert.Contains(
            "Python 3 is required to execute the dependency proof.",
            source,
            StringComparison.Ordinal);
        Assert.Contains(
            ...
            StringComparison.Ordinal);
        Assert.Contains(
            "Dependency proof failed with exit code $LASTEXITCODE.",
            source,
            StringComparison.Ordinal);
    }
