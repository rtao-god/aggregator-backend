...
public sealed class MigrationProofWindowsEntrypointTests
{
    [Fact]
    public void WindowsEntrypointInvokesTheCanonicalPythonOwner()
    {
        var source = Read("tools/run-migration-proof.ps1");
    ...
    public void WindowsEntrypointRequiresPythonThreeAndPropagatesFailure()
    {
        ...
        Assert.Contains(
            "Python 3 is required to execute the migration proof.",
            source,
            StringComparison.Ordinal);
        Assert.Contains(
            ...
            StringComparison.Ordinal);
        Assert.Contains(
            "Migration proof failed with exit code $LASTEXITCODE.",
            source,
            StringComparison.Ordinal);
    }
