...
public sealed class EfCatalogMediaRepository(CatalogMediaDbContext dbContext) : ICatalogMediaRepository
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (workerIdentity.Length > 200 || workerIdentity.Any(char.IsControl))
        {
            throw new ArgumentException(
                "Catalog media worker identity is invalid.",
        ...
        command.Parameters.AddWithValue(
            ...
            (int)CatalogMediaState.Uploaded);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (asset.Id != lease.AssetId ||
            asset.AggregateRevision != lease.StoredAggregateRevision + 1)
        {
            throw Failure(
        ...
        CancellationToken cancellationToken)
    {
        ...
        command.Parameters.AddWithValue(
            "last_error",
            NpgsqlDbType.Text,
            normalizedFailure[..Math.Min(normalizedFailure.Length, 4000)]);
        command.Parameters.AddWithValue(
            "failed_at_utc",
            NpgsqlDbType.TimestampTz,
            failedAtUtc);
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            await using var transaction = await dbContext.Database.BeginTransactionAsync(
                IsolationLevel.Serializable,
                cancellationToken);
            var transactionReplay = await ReadCommandResultAsync(
                commandIdentity,
                cancellationToken);
        ...
        CatalogMediaCommandIdentity identity)
    {
        ...
        return new CatalogMediaCommandResult(
            CatalogMediaPersistenceJson.Deserialize(
                row.ResultDocument,
                row.ResultDigest),
            true);
    }
    ...
    {
        ArgumentNullException.ThrowIfNull(asset);
        return new CatalogMediaAssetRow
        {
            Id = asset.Id,
