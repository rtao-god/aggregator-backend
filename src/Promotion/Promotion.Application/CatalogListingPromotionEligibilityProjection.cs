...
/// <summary>Reconciles active Promotion placements after one authoritative Catalog eligibility change.</summary>
    ...
    IPromotionClock clock)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var projectionResult = await store.ApplyAsync(
            ...
            receivedAtUtc,
            cancellationToken);
        var reconciliationContext = PromotionCommandContext.Continue(
            systemActor,
            message.CorrelationId,
            message.MessageId);
