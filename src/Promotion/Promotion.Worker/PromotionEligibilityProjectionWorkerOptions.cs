...
/// <summary>Validated RabbitMQ contract for the Promotion listing-eligibility projection consumer.</summary>
public sealed record PromotionEligibilityProjectionWorkerOptions
{
    public const string SectionName = "Promotion:EligibilityProjection";

    public required Uri BrokerUri { get; init; }

    public required string Exchange { get; init; }

    public string Queue { get; init; } = "promotion.catalog-listing-eligibility";

    public string DeadLetterExchange { get; init; } = "aggregator.dead-letter";

    public string DeadLetterQueue { get; init; } =
        "promotion.catalog-listing-eligibility.dead-letter";
    ...
    public static PromotionEligibilityProjectionWorkerOptions FromConfiguration(
        IConfiguration configuration)
    {
        ...
        var options = new PromotionEligibilityProjectionWorkerOptions
        {
            BrokerUri = brokerUri,
            Exchange = Require(configuration, "Messaging:Exchange"),
            Queue = Read(configuration, $"{SectionName}:Queue", "promotion.catalog-listing-eligibility"),
    ...
    public void Validate()
    {
        ...
        {
            throw new InvalidOperationException(
                $"{SectionName}:RoutingKey must be the producer-owned Catalog listing eligibility key.");
        }
